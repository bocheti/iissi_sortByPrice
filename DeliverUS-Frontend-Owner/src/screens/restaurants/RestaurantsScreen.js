...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const sort = async (restaurant) => {
    try {
      await toggleSort(restaurant.id)
      await fetchRestaurants()
      showMessage({
        message: `Restaurant ${restaurant.name} now sorting products by ${restaurant.sortByPrice ? 'default' : 'price'}`,
        type: 'success',
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        <TextRegular textStyle={{ alignSelf: 'flex-end', marginRight: '18vh' }}> Currently sorting products by <TextSemiBold>{item.sortByPrice ? 'price' : 'default'}</TextSemiBold></TextRegular>
